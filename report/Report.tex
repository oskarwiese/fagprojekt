...
%	\textbf{TODO: \\ referér til kode og software \\ brug lang tid på nye %metoder, kort tid på gamle metoder}
	...
	\begin{figure}[H]
		...
		\label{fig:permtest}
	\end{figure}\noindent
	From figure \ref{fig:permtest} it is clear that the obtained accuracy is indeed significant and different from the accuracies of the permuted data. This can be evaluated using the p-value. For a permutation test, the p-value is calculated using 
	\[p=\frac{B}{M},\]
	where \textit{B} is the number of extreme permutations (the permutations with values greater than or equal to the observed value) and \textit{M} is the total number of permutations. Since there are no permutations larger than the observed value, this would results in a p-value of 0, which is nonsensical. Thus, the p-value can instead be estimated by 
	\[p=\frac{1}{M}=0.001,\] 
