...
%	As for feature selection every feature was randomly shuffled with re-sampling. If the outputs remained unchanged regardless of the permutation the feature was dropped. Every feature is permuted 1000 times.\\\\
	...
	A key in Hard et Al.'s study is the Receiver Operator Characteristic curve (ROC-curve) of the score function. The ROC-curve shows the relationship between the false positive rate (FPR) and true positive rate(TPR) at different threholds. These curves exist in a two dimensional plane with the true positive rate on the vertical axis the false positive rate on the horizontal axis. The ROC-curves that are considered is the A-conditional ROC-curves which are constructed from the data with the Caucasian and African-American protected groups respectively, to be more precise, ROC-curves are constructed by thresholding the score function and using the confusion matrices described in section \ref{bias_id} to obtain the TPR and FPR.
	\begin{equation*}\label{key}
...
%As for the implementation of Equal Opportunity, it only requires that the TPR is equal for both populations. This was done by using exhaustive search, by choosing a TPR for one population, then subtracting this from the TPR-array for the other population and using \texttt{argmin} to find the one closest to zero.
	...
	\subsection{Validation of Model Accuracy}
	...
	\subsection{Conditional ROC-curves and FPR/TPR Dependency on Threshold}
...
%	\end{figure}
	...
	The implementation of equalized odds below assumes that the ROC-curve belonging to the \texttt{fprs1} and \texttt{tprs1} values is always places above the other ROC-curve in order to save space, as the implementation is similar for having the curves overlap.
	...
	def EqualizedOdds(fprs1, fprs2, tprs1, tprs2, true1, true2, probs1, probs2, ...
	  ...
	  # @Returns:@
	  #    @\textbf{accs[bestIndex](float):} Value of the best accuracy.@
	  #    @\textbf{1-sampleThreshold(float):} The percentage of times the non-1 threshold is sampled.@
	  #    @\textbf{thresholds1[bestIndex](float), thresholds2[bestIndex](float):} The thresholds used for@
	  #    @the two curves.@
	  ...
	  for pointIndex from 0 to points:
	    ...
	    y_pred1Low = if probs1[i] > thres1[i] then 1 otherwise 0 for every value in probs1
	    y_pred1High = if probs1[i] > thres2[i] then 1 otherwise 0 for every value in probs1
	    ...
	    yPred2 = (probs2 > thresholds2[pointIndex]
	    ...
	    accs[i] = acc value in this iteration
	  bestIndex = argmax(accs)
	  return accs[bestIndex], 1-sampleThreshold, thresholds1[bestIndex], thresholds2[bestIndex]
	\end{lstlisting}
	
	\subsubsection{Equal Opportunity}
	\begin{lstlisting}[escapechar=@,escapebegin=\itshape]
	def EqualOpportunity(fprs1, tprs1, fprs2, tprs2, true1, true2, probs1, probs2, thresholds1, thresholds2, points):
	  accs = empty list of size points
	  for pointIndex from 0 to points:
	    x2 = fprs2[pointIndex]
	    y2 = tprs2[pointIndex]
	    ...
	    accs[i] = acc value in this iteration
	  bestIndex = argmax(accs)
	  bestAcc = accs[best_idx]
	  bestThres1 = thresholds1[best_idx]
	  bestThres2 = thresholds2[best_idx]
